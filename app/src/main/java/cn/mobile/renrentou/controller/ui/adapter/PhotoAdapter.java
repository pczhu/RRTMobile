...
import cn.mobile.renrentou.controller.widget.relativelayout.FoldableLayout;
...
//        Picasso.with(holder.mFoldableLayout.getContext()).load(path).into(holder.mImageViewDetail);
        x.image().bind(holder.mImageViewCover,path);
        x.image().bind(holder.mImageViewDetail,path);
    ...
    protected static class PhotoViewHolder extends RecyclerView.ViewHolder {
        ...
        @ViewInject(R.id.imageview_detail)
        protected ImageView mImageViewDetail;

        @ViewInject(R.id.textview_cover)
